...
class FicheroBlockController extends BlockController {
	
	protected $btName = 'Descarga de ficheros';
	protected $btDescription = '';
	protected $btTable = 'btDCFichero';
	...
	public function getSearchableContent() {
		...
		$content[] = $this->field_4_file_linkText;
	...
	public function view() {
		...
		$this->set('field_4_file', (empty($this->field_4_file_fID) ? null : File::getByID($this->field_4_file_fID)));
	...
	public function save($args) {
		...
		$args['field_4_file_fID'] = empty($args['field_4_file_fID']) ? 0 : $args['field_4_file_fID'];
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('FicheroBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('FicheroBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('FicheroBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('FicheroBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('FicheroBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('FicheroBlockController', 'replaceDownloadFileID'),
			$text);
