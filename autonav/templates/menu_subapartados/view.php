...
echo 'LISTADO ISA';
echo '<div class="ccm-block-styles menu-over">'; //opens the top-level menu
echo '<div class="panel-group menu_oculto" id="accordion">';

$i=0;
foreach ($navItems as $ni) {

        if($ni->level==1){
                echo '<div data-toggle="collapse" data-parent="#accordion" href="#collapse'.$i.'"" class="hoverExpand panel panel-default ' . $ni->classes . '">'; 
                echo '<div class="panel-heading"><h4 class="panel-title"><a  data-toggle="collapse" data-parent="#accordion" href="'.$ni->url.'">' . $ni->name . '</a></h4></div>';
                if (!$ni->hasSubmenu) {
                        echo '</div>';
                        $i++;
                }else{
                        echo '<div id="collapse'.$i.'" class=" panel-collapse collapse"><div class="panel-body">'; //opens a dropdown sub-menu//opens a nav item
                        $i++;
                }
                continue;
        }else{

        echo '<a href="' . $ni->url . '" target="' . $ni->target . '">' . $ni->name . '</a><br>';
        }

        echo str_repeat('</div></div></div>', $ni->subDepth);

 }


echo '</div></div>'; //closes the top-level menu

//echo '<script>$(".collapse").collapse("parent")</script>';
