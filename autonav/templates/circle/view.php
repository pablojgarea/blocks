...
echo '<div id="radial_container"><ul class="list">'; //opens the top-level menu

foreach ($navItems as $ni) {

	echo '<li class="item ' . $ni->classes . '">'; //opens a nav item

	echo '<div class="circulo"><a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">' . $ni->name . '</a></div>';

	echo '</li>';
}
echo '<script tyoe="text/javascript">var navItems ='.count($navItems).';</script>';
